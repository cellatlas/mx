...
// display
void displayProgramOptions_norm()
{
    std::cout << "Usage: mx norm [options] sorted mx-files" << std::endl
              << std::endl
              << "Options:" << std::endl
              << "-o, --output          File for output" << std::endl
              << "-a, --axis=<integer>  Axis along which to norm (default = 0)" << std::endl
...
void parseProgramOptions_norm(int argc, char *argv[], MX_opt &opt)
{
    const char *optstring = "o:a:m:p";
...
// validate
bool validateProgramOptions_norm(MX_opt &opt)
{
    bool ret = true;
    return ret;
}
...
void mx_norm(MX_opt &opt)
{
    ...
    std::cerr << nr << " records normd along axis " << axis << std::endl;
}